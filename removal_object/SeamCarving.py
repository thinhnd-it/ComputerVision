...
class SeamCarver:
    ...
    def seams_carving(self):
        ...
        delta_row, delta_col = int(
            self.out_height - self.in_height), int(self.out_width - self.in_width)
    ...
    def cumulative_map_backward(self, energy_map):
        ...
        for row in range(1, m):
            for col in range(n):
                output[row, col] = \
                    energy_map[row, col] + \
                    np.amin(output[row - 1, max(col - 1, 0)
                            : min(col + 2, n - 1)])
        return output
        ...
        for row in range(1, m):
            for col in range(n):
                if col == 0:
                    ...
                    output[row, col] = energy_map[row, col] + \
                        min(e_right, e_up)
                elif col == n - 1:
    ...
    def optimazation_cal_energy(self):
        m, n, ch = image.shape
        output = np.zeros((n, m, ch))
        if ccw:
            image_flip = np.fliplr(image)

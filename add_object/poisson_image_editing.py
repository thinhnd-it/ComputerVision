...
def poisson_edit(source, target, mask, offset):
    ...
    mat_A = mat_A.tocsc()
    print("shape A", mat_A.shape)
    mask_flat = mask.flatten()
    for channel in range(source.shape[2]):
        source_flat = source[y_min:y_max, x_min:x_max, channel].flatten()
        source_fl = source[y_min:y_max, x_min:x_max, channel]
        source_name = 'source' + str(channel) + '.jpg'
        cv2.imwrite(source_name, source_fl)
        target_flat = target[y_min:y_max, x_min:x_max, channel].flatten()

        concat = source_flat*mask_flat + target_flat*(1-mask_flat)
        concat = concat.reshape((y_range, x_range))
        concat_name = 'concat' + str(channel) + '.jpg'
        cv2.imwrite(concat_name, concat)
        # inside the mask:
        # \Delta f = div v = \Delta g
        alpha = 1
        mat_b = laplacian.dot(source_flat)*alpha
        print("shape b", mat_b.shape)
        # outside the mask:
        # f = t
        ...
        filename = 'channel' + str(channel) + '.jpg'
